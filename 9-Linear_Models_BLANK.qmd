...
We can use that model to generate predictions, which will predict log10(pressure), so to get just the regular pressure, we can do 10^prediction. If you used log() instead of log10() the inverse would be exp(). 
